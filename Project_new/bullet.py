...
class BossBullet:
    ...
    def __init__(self, x, y, bg):
        ...
        if BossBullet.image == None:
            BossBullet.image = load_image('resource/Boss/Boss_1_Ball.png')

    def update(self, frame_time):
        def clamp(minimum, x, maximum):
            return max(minimum, min(x, maximum))
        speed = BossBullet.BULLET_SPEED_PPS * frame_time
        self.x -= speed
        self.total_frame += BossBullet.FRAMES_PER_ACTION * BossBullet.ACTION_PER_TIME * frame_time
        self.frame = int(self.total_frame) % 3
        self.x = clamp(-100, self.x, self.bg.w + 100)

    def draw(self):
        self.sx = self.x - self.bg.left
        self.image.clip_draw(self.frame * 100, 0, 100, 35, self.sx, self.y)
        # self.draw_bb()
